...
<div class="output__container">
    ...
    <h2 class="output__logo">Пользователи с полем User Name - <?php echo $username; ?></h2>
    ...
    $messages = mysqli_query($connection, "SELECT * FROM `form` WHERE `username` = '$username' ORDER BY `id` DESC LIMIT $offset,$per_page");
    ...
    if (mysqli_num_rows($messages) > 0) {
        ...
        if ($page > 1) {
            echo '<a href="/username.php?page=' . ($page - 1) . '" class="output__btn">&laquo; Прошлая страница</a> ';
        }
        if ($page < $total_pages) {
            echo '<a href="/username.php?page=' . ($page + 1) . '" class="output__btn">Следующая страница &raquo;</a>';
        }
        echo '<div/>';
    }
    ?>
