...
<div class="output__container">
    ...
    ?>
    <h2 class="output__logo">Пользователи с полем E-mail - <?php echo $email; ?></h2>
    ...
    $total_count_q = mysqli_query($connection, "SELECT COUNT(`id`) AS `total_count` FROM `form` WHERE `e-mail` = '$email'");
    ...
    $messages = mysqli_query($connection, "SELECT * FROM `form` WHERE `e-mail` = '$email' ORDER BY `id` DESC LIMIT $offset,$per_page");
    ...
    if (mysqli_num_rows($messages) > 0) {
        ...
        if ($page > 1) {
            echo '<a href="/email.php?page=' . ($page - 1) . '" class="output__btn">&laquo; Прошлая страница</a> ';
        }
        if ($page < $total_pages) {
            echo '<a href="/email.php?page=' . ($page + 1) . '" class="output__btn">Следующая страница &raquo;</a>';
        }
        echo '<div/>';
    }
    ?>
